...
def check_config_exists():
    ...
    config_exists = harty_config.check_config_exists()
    if config_exists:
...
def get_save_directory():
    ...
    save_dir_message = "\nPlease provide the directory where your Harty data should be stored. " \
                       "If you are planning on using Harty on multiple devices, it is recommended " \
...
def create_symlink():
    ...
    symlink_message = "\nProvide directory for symlink for easy access to 'harty' command. " \
    ...
    if symlink_path:
        ...
        print("\nCreating symlink at: {}".format(symlink_path + "/harty"))

        if not DRY_RUN:
            os.symlink(top_dir + "/harty.py", symlink_path + "/harty")
    else:
        print("\nNo symlink created")
...
def setup_config_file(base_save_dir, dry_run=False):
    ...
    config.set('file_paths', 'sql_path', '%(output_base)s/harty_data.sqlite')
    config.set('file_paths', 'output_base', base_save_dir)

    config.set('file_paths', 'figures', '%(output_base)s/figures')
    config.set('file_paths', 'html', '%(output_base)s/harty.html')

    setup_config_file_settings(config)
    harty_base_dir = harty_config.get_base_dir()
    conf_path = '{}/{}'.format(harty_base_dir, 'harty.conf')
...
def setup_config_file_settings(config):
    ...
    if chosen_log_type not in ['pomo', 'block']:
        ...
        print("You can adjust this later by editing the harty.conf configuration file")
